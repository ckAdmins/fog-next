...
func (_q *GroupMemberQuery) QueryGroup() *GroupQuery {
...
func (_q *GroupMemberQuery) QueryHost() *HostQuery {
...
func (_q *GroupMemberQuery) WithGroup(opts ...func(*GroupQuery)) *GroupMemberQuery {
	query := (&GroupClient{config: _q.config}).Query()
...
func (_q *GroupMemberQuery) WithHost(opts ...func(*HostQuery)) *GroupMemberQuery {
	query := (&HostClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupMemberQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*GroupMember, error) {
	...
	if query := _q.withHost; query != nil {
		if err := _q.loadHost(ctx, query, nodes, nil,
			func(n *GroupMember, e *Host) { n.Edges.Host = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GroupMemberQuery) loadGroup(ctx context.Context, query *GroupQuery, nodes []*GroupMember, init func(*GroupMember), assign func(*GroupMember, *Group)) error {
...
func (_q *GroupMemberQuery) loadHost(ctx context.Context, query *HostQuery, nodes []*GroupMember, init func(*GroupMember), assign func(*GroupMember, *Host)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*GroupMember)
	for i := range nodes {
		fk := nodes[i].HostID
...
func (_q *GroupMemberQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withHost != nil {
			_spec.Node.AddColumnOnce(groupmember.FieldHostID)
		}
	}
