...
type PrinterQuery struct {
	...
	withAssocs      *PrinterAssocQuery
	withNamedAssocs map[string]*PrinterAssocQuery
...
func (_q *PrinterQuery) QueryAssocs() *PrinterAssocQuery {
	query := (&PrinterAssocClient{config: _q.config}).Query()
...
func (_q *PrinterQuery) WithAssocs(opts ...func(*PrinterAssocQuery)) *PrinterQuery {
	query := (&PrinterAssocClient{config: _q.config}).Query()
...
func (_q *PrinterQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Printer, error) {
	...
	if query := _q.withAssocs; query != nil {
		if err := _q.loadAssocs(ctx, query, nodes,
			func(n *Printer) { n.Edges.Assocs = []*PrinterAssoc{} },
			func(n *Printer, e *PrinterAssoc) { n.Edges.Assocs = append(n.Edges.Assocs, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedAssocs {
		if err := _q.loadAssocs(ctx, query, nodes,
			func(n *Printer) { n.appendNamedAssocs(name) },
			func(n *Printer, e *PrinterAssoc) { n.appendNamedAssocs(name, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PrinterQuery) loadAssocs(ctx context.Context, query *PrinterAssocQuery, nodes []*Printer, init func(*Printer), assign func(*Printer, *PrinterAssoc)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(printerassoc.FieldPrinterID)
	}
	query.Where(predicate.PrinterAssoc(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(printer.AssocsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "printer_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *PrinterQuery) WithNamedAssocs(name string, opts ...func(*PrinterAssocQuery)) *PrinterQuery {
	query := (&PrinterAssocClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedAssocs == nil {
		_q.withNamedAssocs = make(map[string]*PrinterAssocQuery)
	}
