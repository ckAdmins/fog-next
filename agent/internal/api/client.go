...
type Client struct {
	...
	program      *tea.Program // optional Bubble Tea program for TUI status updates
}

// New creates a Client pointed at baseURL (e.g. "http://10.0.0.1").
// If p is non-nil, status updates are sent to the TUI dashboard.
func New(baseURL string, p *tea.Program) *Client {
	c := &Client{
	...
	if p != nil {
		tui.SendMsg(p, tui.ServerMsg(c.base))
	}
	return c
}
...
type HandshakeResponse struct {
	...
	// resized during deploy even when ImageType is "resizable".
	FixedSizePartitions []int `json:"fixedSizePartitions,omitempty"`
	// PartTypes carries the filesystem type for each partition (1:1 with PartCount).
	PartTypes []string `json:"partTypes,omitempty"`
	// PartNumbers carries the actual kernel partition number for each entry
	// (e.g. 1, 14, 15 for a Debian cloud image). 1:1 with PartCount.
...
func (c *Client) Handshake(ctx context.Context, req HandshakeRequest) (*HandshakeResponse, error) {
	if c.program != nil {
		tui.SendMsg(c.program, tui.PhaseMsg("Performing handshake with server..."))
	}
	...
	if c.program != nil {
		tui.SendMsg(c.program, tui.ConnectedMsg(true))
		tui.SendMsg(c.program, tui.TaskMsg{
			TaskID:  resp.TaskID,
			ImageID: resp.ImageID,
			Action:  resp.Action,
		})
		tui.SendMsg(c.program, tui.PhaseMsg(fmt.Sprintf("Handshake OK — action: %s", resp.Action)))
	}
	return &resp, nil
}
...
// SetImageMeta stores capture-time metadata for an image.
...
// If offset is > 0, the server appends to the existing file rather than
// replacing it (resume support for capture retries).
