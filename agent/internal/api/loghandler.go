...
// FlushAll drains every buffered log entry in a single synchronous HTTP call
// and blocks until the server responds (or the request times out).  Call this
// immediately before process exit to guarantee logs are received — the
// background drainLoop runs on a 1 s tick which is too slow to catch the
// final entries produced during a fast failure.
func (h *RemoteHandler) FlushAll() {
	h.mu.Lock()
	client := h.client
	taskID := h.taskID
	if client == nil || len(h.buf) == 0 {
		h.mu.Unlock()
		return
	}
	batch := make([]LogEntry, len(h.buf))
	copy(batch, h.buf)
	h.buf = h.buf[:0]
	h.mu.Unlock()
	...
	defer cancel()
	if err := client.SendLogs(ctx, req); err != nil {
		fmt.Fprintf(delegateWriter(h.delegate), "time=%s level=WARN msg=\"final log flush failed\" err=%v\n",
